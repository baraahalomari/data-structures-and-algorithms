...
const addPurchases = (arr) => {
  const total = arr.reduce((acc,val,idx)=>{
...
const returnNames = (arr) => {
  let newName = starWarsData.reduce((acc,val,idx)=>{
    acc.push(val.name);
    return acc;
  },[]);
  return newName;
...
const reversedString = (str) => {
  var splittedString = str.split("");
    var strLength = splittedString.length - 1;
  let newName = splittedString.reduce((acc,val,idx)=>{
   
    acc=acc+splittedString[strLength - idx];
    return acc;
  },"");
  return newName;
};
