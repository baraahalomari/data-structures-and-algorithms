...
const screenForNames = (arr) => {
  return arr.filter(ele => {
    return /^(Dr||Ms||Mr||Mrs).\s[A-Z]/.test(ele);
...
const toTitleCase = (arr) => {
  ...
  let newArr = [];
  arr.forEach(ele => {
    let newArr2 = ele.replace(testArr, ele[0].toUpperCase())
    newArr.push(newArr2);
  })
  return newArr;
};
...
const detectTicTacToeWin = (board) => {
  for (let i = 0; i < 9; i++) {
    ...
    if (board[i][i] === board[i][i + 1] && board[i][i] === board[i][i + 2]  && board[i][i] !== '') {
      return true;
    } else {
      for (let j = 1; j < 9; j++) {
        if (board[j][i] === board[j][j] && board[j][i] === board[j][j + 1]  && board[j][j] !== '') {
          return true;
        } else {
          for (let a = 2; a < 9; a++) {
            console.log(board[2][a]);
            if (board[a][i] === board[a][j] && board[a][i] === board[a][a]) {
              return true;
            } else if (board[i][i] === board[j][i] && board[i][i] === board[a][i]) {

              return true;
            } else if (board[i][j] === board[j][j] && board[a][j] === board[i][j]) {
              return true;
            } else if (board[i][a] === board[j][a] && board[a][a] === board[i][a]) {
              return true;
            }  else if (board[i][i] === board[j][j] && board[i][i] === board[a][a]) {
                return true;
              } else if (board[i][a] === board[a][i] && board[i][a] === board[j][j]) {
                return true;
            } else {
              return false;
            }
